...
const getClassMeta = (classId: string, t: any) => {
  for (const group of CLASS_GROUPS_CONFIG) {
    ...
    if (opt) {
      const fullLabel = t(opt.labelKey);
      return {
        fullLabel: fullLabel,
  ...
  return { fullLabel: classId, groupLabel: '?', color: '#94a3b8', borderColor: 'rgba(148, 163, 184, 0.3)', bgColor: 'rgba(148, 163, 184, 0.1)' };
};
                         ...
                         <Grid container spacing={2}>
                            {CLASS_GROUPS_CONFIG.map(group => (
                               <Grid size={{ xs: 6, md: 3 }} key={group.id}>
                                  <Typography variant="caption" color="text.secondary" fontWeight={900} textTransform="uppercase" display="block" mb={1}>{t(group.labelKey)}</Typography>
                                  <Stack spacing={1}>
