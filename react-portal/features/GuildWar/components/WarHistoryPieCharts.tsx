...
export function WarHistoryPieCharts({ data }: WarHistoryPieChartsProps) {
  ...
  const participationData = useMemo(() => {
    ...
    data.forEach(war => {
      war.member_stats?.forEach(member => {
        const current = memberParticipation.get(member.user_id) || { username: member.username, count: 0 };
        memberParticipation.set(member.user_id, { ...current, count: current.count + 1 });
      });
    });
  ...
  const combatRoleData = useMemo(() => {
    ...
    data.forEach(war => {
      war.member_stats?.forEach(member => {
        const current = memberStats.get(member.user_id) || {
