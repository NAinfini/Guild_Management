...
type ApiKeyNext = (context: ApiKeyContext) => Response | Promise<Response>;
 ...
 */
export async function withAPIKeyAuth(context: ApiKeyContext, next: ApiKeyNext): Promise<Response> {
  ...
  ).bind(keyData.userId).first<User>();
...
export function requireScope(scope: string) {
  return (context: ApiKeyContext, next: ApiKeyNext): Response | Promise<Response> => {
    const apiKeyData = context.data?.apiKey as { scopes?: string[] } | undefined;
