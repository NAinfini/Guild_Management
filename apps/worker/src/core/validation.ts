...
export const batchAnnouncementActionSchema = z.object({
  announcementIds: z.array(z.string()).min(1).max(100),
  action: z.enum(['archive', 'unarchive', 'delete', 'pin', 'unpin']),
});
...
export const updateUsernameSchema = z.object({
  newUsername: z.string().min(1).max(50),
  password: z.string().min(1).max(200),
});
...
const assignMemberItem = z.object({
  userId: z.string().optional(),
  user_id: z.string().optional(),
  teamId: z.string().optional(), // If null, assign to pool
  team_id: z.string().optional(),
  roleTag: z.string().max(32).optional(),
  role_tag: z.string().max(32).optional(),
});

export const assignMemberSchema = z.union([
  assignMemberItem,
  z.object({
    operations: z.array(assignMemberItem).min(1),
  }),
]);
