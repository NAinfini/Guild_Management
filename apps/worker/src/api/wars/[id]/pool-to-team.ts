/**
 * Guild War - Move Member from Pool to Team
 * POST /api/wars/[id]/pool-to-team
 */

import type { Env } from '../../../core/types';
import { createEndpoint } from '../../../core/endpoint-factory';
import { utcNow, createAuditLog, successResponse } from '../../../core/utils';
import { NotFoundError } from '../../../core/errors';

// SingleMove interface updated to include roleTag
interface SingleMove {
  userId: string;
  teamId: string;
  roleTag?: string;
}

interface BatchMove {
  moves: SingleMove[];
}

export const onRequestPost = createEndpoint<{ message: string; count: number }, SingleMove | BatchMove>({
  auth: 'moderator',
  
  handler: async ({ env, user, params, body }) => {
    if (!user) throw new Error('User not authenticated');

    const eventId = params.id;
    const now = utcNow();

    // Get War Details
    const warHistory = await env.DB.prepare('SELECT war_id FROM war_history WHERE event_id = ?').bind(eventId).first<{ war_id: string }>();
    if (!warHistory) throw new NotFoundError('War');
    const warId = warHistory.war_id;

    const moves: SingleMove[] = [];
    if ('moves' in body && Array.isArray(body.moves)) {
      moves.push(...body.moves);
    } else if ('userId' in body && 'teamId' in body) {
      moves.push(body as SingleMove);
    }

    if (moves.length === 0) throw new Error('No moves specified');

    // Get all Team IDs linked to this Event
    const { results: eventTeams } = await env.DB.prepare('SELECT team_id FROM event_teams WHERE event_id = ?').bind(eventId).all<{ team_id: string }>();
    const validTeamIds = new Set(eventTeams.map(et => et.team_id));

    // Sort Order Optimization
    const targetTeamIds = [...new Set(moves.map(m => m.teamId))];
    const sortOrders = new Map<string, number>();
    for (const tid of targetTeamIds) {
        if (!validTeamIds.has(tid)) throw new Error(`Target team ${tid} is not part of this event`);
        const max = await env.DB.prepare('SELECT MAX(sort_order) as max_sort FROM team_members WHERE team_id = ?').bind(tid).first<{ max_sort: number }>();
        sortOrders.set(tid, max?.max_sort || 0);
    }

    // Logic: Move from Current Team -> Target Team (Update)
    // We update the team_id and role_tag for the user's existing membership in this event
    const stmtMoveMember = env.DB.prepare(`
        UPDATE team_members 
        SET team_id = ?, sort_order = ?, role_tag = ?
        WHERE user_id = ? AND team_id IN (SELECT team_id FROM event_teams WHERE event_id = ?)
    `);

    const batch: any[] = [];
    let successCount = 0;
    
    for (const move of moves) {
        // Increment sort order
        const currentSort = sortOrders.get(move.teamId)! + 1;
        sortOrders.set(move.teamId, currentSort);

        // Update team membership
        batch.push(stmtMoveMember.bind(move.teamId, currentSort, move.roleTag || null, move.userId, eventId));
        successCount++;
    }
    
    // Updating War Timestamp
    batch.push(env.DB.prepare('UPDATE war_history SET updated_at_utc = ? WHERE war_id = ?').bind(now, warId));

    await env.DB.batch(batch);

    await createAuditLog(
      env.DB, 'war', 'pool_to_team_batch', user.user_id, warId,
      `Moved ${successCount} members to teams`,
      JSON.stringify({ count: successCount, moves })
    );

    return successResponse({ message: `Moved ${successCount} members to teams`, count: successCount });
  }
});
