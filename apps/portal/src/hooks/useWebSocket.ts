  ...
  const handleMessage = useCallback((message: WebSocketMessage) => {
    ...
    try {
        if (entity === 'members') {
            ...
            // Keep local auth Zustand user in sync for self updates.
            const currentUser = useAuthStore.getState().user;
            if (currentUser) {
              const selfUpdate = mapped.find((m: any) => m.id === currentUser.id);
              if (selfUpdate) {
                useAuthStore.getState().setUser({ ...currentUser, ...selfUpdate });
              }
            }
