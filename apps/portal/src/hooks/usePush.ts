...
export function usePush(entities: string[] = ['wars', 'events', 'announcements', 'members']) {
  ...
  useEffect(() => {
    ...
    const devOrigin = import.meta.env.VITE_DEV_API_ORIGIN || 'https://guild-management.na-infini.workers.dev';
    const configuredApiBase = import.meta.env.VITE_API_BASE_URL || (isDev ? `${devOrigin}/api` : '/api');
    const pushUrl = new URL(`${configuredApiBase}/push?${qs.toString()}`, window.location.origin).toString();
    const es = new EventSource(pushUrl, { withCredentials: true });
    ...
    es.onerror = () => {
      // Keep the connection open; EventSource handles automatic retries.
    };
