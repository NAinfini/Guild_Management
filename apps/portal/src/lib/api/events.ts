...
export const eventsAPI = {
   ...
   * Toggle Pin
   */
  togglePin: async (id: string): Promise<{ isPinned: boolean; message: string }> => {
      return typedAPI.events.togglePin<{ isPinned: boolean; message: string }>({ params: { id }, body: {} });
  },
   ...
   * Toggle Lock
   */
    toggleLock: async (id: string): Promise<{ isLocked: boolean; message: string }> => {
        return typedAPI.events.toggleLock<{ isLocked: boolean; message: string }>({ params: { id }, body: {} });
    },

    toggleArchive: async (id: string): Promise<{ isArchived: boolean; message: string }> => {
        return typedAPI.events.toggleArchive<{ isArchived: boolean; message: string }>({ params: { id }, body: {} });
    },
