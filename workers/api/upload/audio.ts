  ...
  return withAuth(context, async (authContext) => {
    ...
    try {
      ...
      if (!file.type.includes('audio')) {
        return badRequestResponse('Invalid file type. Must be an audio file.');
      }

      // Validate file size (max 20MB)
      const maxSize = 20 * 1024 * 1024;
      if (file.size > maxSize) {
        return badRequestResponse('File too large. Maximum size is 20MB.');
      }
      ...
      return errorResponse('INTERNAL_ERROR', 'An error occurred while uploading audio', 500);
    }
  });
};
